...
| **I21** ✅ | Phase 3 公开面 | 默认页 **`posts` → `index`**：改 `default` / `page` / wrapper 的 `brand.src`（最易漏，§14b）/ `blocklet.yaml` binding 的 `page:` 四处引用；12 个 `posts.*` i18n 键随命名空间变 `index.*`（键值不变，总数仍 243）；`man/posts.yaml` → `man/index.yaml`。实测并记入 `arc-contracts.md` §18.1 两处**工具不清理**的残留：`generate` 不删旧页生成物（且 `--check` 不报警；残留的 `pages/posts.json` 会让死键守卫少报 2 个键，删掉后才报全 12 个）、`arc blocklet build` 不清理 dist 旧产物 —— 均手工删除。对外 URL 不变：`/` 由 `default` 决定、`/posts` 由 binding 的 `path` 决定，与页面名解耦 | generate --check + validate + check + test（172）+ build（283 文件）+ 运行时 WS 载荷实测 `defaultPage: index`、`page: index`、`brand.src: index`（浏览器工具本轮不可用） |
| **I20** ✅ | Phase 3 公开面 | **locale 治理**：把 `.aup/locales/*.json` 的真实生命周期（页面 `i18n {}` 是源、generate 只合并不清理、`wrapper.*` 是唯一手写部分）写入 `arc-contracts.md` §18；9 个"被引用但无声明"的键改为在所属页声明（4 个 `admin.*-ok` toast + 5 个 heroes-admin 里写死的 `$t(admin.hero-*/cat-*)` → `heroes-admin.*`），手写部分只剩 14 个 `wrapper.*`；清掉 **78 个死键**（316→243，来自 `admin` 页面拆分与侧栏改指 `wrapper.nav-*` 后的残留）。新增 `scripts/arcblog-locales.mjs`（report / `--check` / `--write`）；`arc dsl generate --check` 与死键检查都进 `npm test`，两者均做反证（植入假键、制造漂移各自变红并点名） | generate --check + validate + test（172）+ build（283 文件）+ 浏览器实测 14 页无 `$t()` 字面量 |
| **I19** ✅ | Phase 3 公开面 | 控制台**按功能拆分页面**：`admin` 只留文章（工作室头部/生命周期/外观 + 已发布 + 草稿，194→97 行）、`operations` 只留节点与网络（健康/发现/Hub，165→89 行）；新增 `heroes-admin`（Hero + 分类）、`ops-admin`（媒体索引 + Agent 授权）、`economy-admin`（策略/商品/访问授权）；`dashboard` 瘦身为落地页并清 14 个死键。侧栏**按功能分 3 组**（内容/运营/经济），菜单模型抽到 `scripts/console-nav.mjs`，由 `scripts/arcblog-console-nav.mjs` 生成进 6 个页面（AUP 无 include），`--check` 进测试套件（做了漂移反证） | validate + generate + test（167）+ build（282 文件）+ 6 页浏览器实测 |
| **I18** ✅ | Phase 3 公开面 | 用户菜单 3 个自定义入口合并为 **1 个**（管理后台→仪表盘）；三个管理页改为**左侧菜单 + 右侧内容**外壳（侧栏 210px、当前项 `variant=primary`、应用内切换）；侧栏标签改为跨命名空间引用 `$t(wrapper.nav-*)`（单一来源，替掉 9 处页面级重复）；i18n 守卫的死键检查扩大到整个已编译应用（并做了反证） | generate + validate + test（164）+ build（272 文件）+ 浏览器实测三页 |
| **I17** ✅ | Phase 3 公开面 | 首页 hero 圆角与 frame 面板对齐（`--radius-lg`）；示例文章补齐英文总览正文+封面图（正文页封面亦可渲染）；列表卡片重做（字号/间距/摘要、无图不渲染封面、`flexGrow:0` 修拉伸）；排查并修掉 filter 下推导致的**列表静默丢记录**（改 propBind / 去 filter，见 arc-contracts §15） | generate + validate + test（164）+ build（266 文件）+ 浏览器逐项实测 |
| **I16** ✅ | Phase 3 公开面 | 首页 hero 换为**自动播放轮播**：实测 `afs-list layout=slideshow` 无计时属性 → 走自定义组件 + ARC 自带 `photo-story` widget（vendor 后与 init 拼接生成 `script.js`）、`frame aspectRatio` 定尺寸、AFS `includeContent` 读 hero 记录、跟随宿主主题、无图幻灯片优雅降级；含生成物同步测试 | generate + validate + test（164）+ check + build（265 文件）+ 浏览器实测 autoplay 推进 |
