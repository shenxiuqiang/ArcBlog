...
- `/instance/app/arcblog/config/trusted-hubs/<didHash>.json` — a Hub's **public** key (`hubDid`, `publicKey`, `label`); **admin-only** (a guest grant on the `config` prefix also exposed the directory listing of admin-only children — see `arc-contracts.md` §13). Private keys are never stored in AFS — `scripts/arcblog-attribution.mjs keygen` writes them to a local file with mode 0600.
- `/instance/app/arcblog/economy/attributions/<contentSlug>-<didHash>.json` — stored Discovery Contexts (spec §32) with a `verified` flag; **admin-only**. Written by `verify --store` only after the signature checks out against the key already trusted for that `hubDid`.
