...
- `/instance/app/arcblog/config/trusted-hubs/<didHash>.json` — a Hub's **public** key (`hubDid`, `publicKey`, `label`); guest-readable (public keys are public by definition). Private keys are never stored in AFS — `scripts/arcblog-attribution.mjs keygen` writes them to a local file with mode 0600.
