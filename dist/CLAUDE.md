...
node scripts/arcblog-console-nav.mjs         # regenerate the console sidebars from scripts/console-nav.mjs
node scripts/arcblog-console-nav.mjs --check # verify every console page carries that menu (part of npm test)
...
- `.aup/` — the app itself: `app.aup` (app shell + page definitions with inline i18n en/zh), `man/*.yaml` (management docs), `pages/*.json`, `wrapper.json/.aup` (app chrome), `locales/`. Pages: about, admin (Creator Studio posts: the published/draft lists and their lifecycle), author, compose, compose-edit, dashboard (landing page: node status, recent posts, quick actions), economy-admin (split policy, products, access grants), heroes-admin (hero carousel + category taxonomy), operations (node health, discovery, registered Hubs), ops-admin (media index + agent grants), posts, preview, reader, settings. The **six console pages** (dashboard / admin / heroes-admin / operations / ops-admin / economy-admin) share a **left-menu + right-content shell** in three groups — 内容 (dashboard, admin, heroes-admin), 运营 (operations, ops-admin), 经济 (economy-admin): the user menu carries one entry (`nav-console` -> dashboard) and each page renders a `row console-shell` with a `view console-nav` sidebar (the current item `variant=primary`) beside a `view console-pane` holding the page content. The sidebar markup is **generated into every page** by `node scripts/arcblog-console-nav.mjs` from the model in `scripts/console-nav.mjs` (`scripts/arcblog-console-nav.test.mjs` fails when a page drifts), and its labels are `$t(wrapper.nav-*)` keys, so the copy lives once in `wrapper.aup` + `locales/`. i18n keys are otherwise **page-scoped** — a key defined on one page cannot be referenced from another (the wrapper namespace is the exception).
...
- **No DSL fragments**: AUP has no include/partial primitive, and expressions support no comparisons, so a shell that differs per page (or an "active" nav item) must be repeated in each page — which is why the console sidebar is *generated* into all six pages by `scripts/arcblog-console-nav.mjs` from `scripts/console-nav.mjs`, with `--check` in the test suite (`arc-contracts.md` §16).
...
- **i18n**: wrapper 的 `i18n {}` 块用 `:key` 引用（`label: :nav-about`）编译成 `$t(wrapper.nav-about)`；`arc dsl generate` **不生成** `wrapper.*` 的 locale key（合并语义，只保留已存在的），因此这些 key 手工维护在 `.aup/locales/*.json` 里，并由 `scripts/arcblog-i18n.test.mjs` 守住。改文案后必须 `arc service restart` 才对线上生效。
