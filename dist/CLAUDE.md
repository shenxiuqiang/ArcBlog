...
- `.aup/` — the app itself: `app.aup` (app shell + page definitions with inline i18n en/zh), `man/*.yaml` (management docs), `pages/*.json`, `wrapper.json/.aup` (app chrome), `locales/`. Pages: about, admin (Creator Studio), author, compose, compose-edit, dashboard, **operations** (read-only console for health/Hubs/economy/media/agent grants), posts, preview, reader, settings. i18n keys are **page-scoped** — a key defined on one page cannot be referenced from another.
