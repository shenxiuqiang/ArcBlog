...
- `.aup/` — the app itself: `app.aup` (app shell + page definitions with inline i18n en/zh), `man/*.yaml` (management docs), `pages/*.json`, `wrapper.json/.aup` (app chrome), `locales/`. Pages: about, admin (Creator Studio — posts, heroes, **category management**), author, compose, compose-edit, dashboard, **operations** (read-only console for health/Hubs/economy/media/agent grants), posts, preview, reader, settings. The three management pages (dashboard / admin / operations) share a **left-menu + right-content shell**: the user menu carries one entry (`nav-console` -> dashboard) and each management page renders a `row console-shell` with a `view console-nav` sidebar (3 items, the current one `variant=primary`) beside a `view console-pane` holding the page content. i18n keys are **page-scoped** — a key defined on one page cannot be referenced from another.
...
- **No DSL fragments**: AUP has no include/partial primitive, and expressions support no comparisons, so a shell that differs per page (or an "active" nav item) must be repeated in each page — that is why the sidebar markup appears three times (`arc-contracts.md` §16).
