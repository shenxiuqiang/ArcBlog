...
- `world/*.yaml` — AFS record schemas (`post`, `node`, `node-identity`, `category`, `media`, `role-config`, `settlement-policy`, `product`, `order`, `settlement`, `ledger-entry`).
...
- `/instance/app/arcblog/economy/{policies,products,orders,settlements,ledger}/` — the economy (spec §41–§43/§91). The policy is public; products are guest-readable; orders, settlements and ledger entries are **admin-only**. Managed by `scripts/arcblog-economy.mjs`; payment and settlement are separate steps, and the ledger is append-only with deterministic ids.
